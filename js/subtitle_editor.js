...
function syncTime(index, type) {
    if(!player || !player.getCurrentTime) { alert("Video not ready"); return; }
...
function fetchFileList() {
    ...
    // CHANGED: Default to gh-pages if empty
    const b = document.getElementById('ghBranch').value.trim() || 'gh-pages'; 
    ...
    fetch(url, { headers: headers })
        .then(res => {
            if(!res.ok) {
                if(res.status === 404) throw new Error("Folder/Branch not found (404). Check branch name.");
                if(res.status === 401) throw new Error("Auth required (401). Check token.");
        ...
        .then(data => {
            ...
            jsonFiles.forEach(file => {
                fileUrlMap[file.name] = file.download_url;
...
function uploadToGithub() {
    ...
    // CHANGED: Default to gh-pages
    const b=document.getElementById('ghBranch').value || 'gh-pages';
