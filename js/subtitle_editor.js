...
let fileUrlMap = {}; // Stores download URLs
...
window.onload = function() {
    ...
    ['ghUser', 'ghRepo', 'ghToken', 'ghBranch'].forEach(k => { 
            ...
            <div class="sub-card" id="card-${i}">
                <div class="card-header">
                    ...
                    <div style="display:flex; gap:10px;">
                        <button class="btn-icon" style="color:#27ae60; font-size:14px;" onclick="player.seekTo(${s.start}, true)" title="Play Segment">▶</button>
                        <button class="btn-icon" style="color:#c0392b" onclick="deleteLine(${i})" title="Delete Line">✖</button>
                    </div>
                </div>
                ...
                <div class="time-control-area">
                    ...
                    <div class="time-row">
                        ...
                        <button class="btn-sync" onclick="syncTime(${i}, 'start')" title="Sync to current video time">Sync</button>
                        <input type="text" class="time-inp" value="${secToTime(s.start)}" onchange="updateData(${i}, 'start', this.value)">
                        <div class="btn-group">
                            <button class="time-btn" onclick="adjustTime(${i}, 'start', -1)">-1</button>
                            <button class="time-btn" onclick="adjustTime(${i}, 'start', -0.1)">-.1</button>
                            <button class="time-btn" onclick="adjustTime(${i}, 'start', 0.1)">+.1</button>
                            <button class="time-btn" onclick="adjustTime(${i}, 'start', 1)">+1</button>
                        </div>
                    </div>
                    ...
                    <div class="time-row">
                        ...
                        <button class="btn-sync" onclick="syncTime(${i}, 'end')" title="Sync to current video time">Sync</button>
                        <input type="text" class="time-inp" value="${secToTime(s.end)}" onchange="updateData(${i}, 'end', this.value)">
                        <div class="btn-group">
                            <button class="time-btn" onclick="adjustTime(${i}, 'end', -1)">-1</button>
                            <button class="time-btn" onclick="adjustTime(${i}, 'end', -0.1)">-.1</button>
                            <button class="time-btn" onclick="adjustTime(${i}, 'end', 0.1)">+.1</button>
                            <button class="time-btn" onclick="adjustTime(${i}, 'end', 1)">+1</button>
                        </div>
                    </div>
...
function adjustTime(index, type, amount) {
    ...
    // Sort if start time changed? 
    // Usually annoying while editing, so we just re-render specifically
    renderEditor(); 
    
    // Optional: Jump to that time
    if(type === 'start' && player && player.seekTo) {
        player.seekTo(subtitleData[index][type], true);
    }
}

// Sync to current player time
...
// Format: 00:00:00,000 --> Seconds
function timeToSec(t) { 
    const p = t.replace(',','.').split(':'); 
    return p.length === 3 ? (+p[0])*3600 + (+p[1])*60 + (+p[2]) : 0; 
}

// Format: Seconds --> 00:00:00,000
...
function fetchFileList() {
    ...
    const b = document.getElementById('ghBranch').value.trim() || 'main'; 
    ...
    localStorage.setItem('ghBranch', b);
    if(t) localStorage.setItem('ghToken', t);

    select.innerHTML = "<option>Loading...</option>";

    const url = `https://api.github.com/repos/${u}/${r}/contents/data?ref=${b}`;
    const headers = t ? { 'Authorization': `token ${t}` } : {};

    fetch(url, { headers: headers })
        .then(res => {
            if(!res.ok) {
                if(res.status === 404) throw new Error("Folder/Branch not found (404)");
                if(res.status === 401) throw new Error("Auth required (401)");
        ...
        .then(data => {
            ...
            jsonFiles.forEach(file => {
                fileUrlMap[file.name] = file.download_url; // Store absolute URL
...
function fetchFromGithub() {
    ...
    if(!u || !r || !f) { alert("Fill all fields."); return; }
    if(!f.endsWith('.json')) f += '.json';
    
    // Use stored Map URL or fallback
    const url = fileUrlMap[f] || `https://raw.githubusercontent.com/${u}/${r}/${b}/data/${f}`;
    ...
    fetch(url)
        ...
        .then(d => { 
            processJsonData(d); 
            btn.innerText = "✅"; setTimeout(()=>btn.innerText=txt, 1500); 
        })
        .catch(e => { 
...
function uploadToGithub() {
    ...
    localStorage.setItem('ghBranch', b); localStorage.setItem('ghToken', t);
    
    const url = `https://api.github.com/repos/${u}/${r}/contents/data/${id}.json`;
    const content = btoa(unescape(encodeURIComponent(buildJson())));
    
    fetch(url + `?ref=${b}`, { headers:{'Authorization':`token ${t}`} })
        .then(res => { return res.status === 404 ? { sha: null } : res.json(); })
        .then(d => { 
            fetch(url, {
                method: 'PUT', 
                headers: { 'Authorization': `token ${t}`, 'Content-Type': 'application/json' }, 
                body: JSON.stringify({ message: "Update via Editor", content: content, branch: b, sha: d.sha })
...
function addSubtitleLine() { 
    ...
    subtitleData.sort((a,b)=>a.start-b.start); 
    renderEditor(); 
    setTimeout(() => { const el = document.getElementById('rows-area'); el.scrollTop = el.scrollHeight; }, 100);
}
...
function applyFormat(index, startTag, endTag) {
    ...
    el.value = txt.substring(0, start) + startTag + txt.substring(start, end) + endTag + txt.substring(end);
    updateData(index, 'text', el.value);
...
function downloadJson() { 
    ...
    a.download = (document.getElementById('targetFilename').value || "sub").replace('.json','') + ".json"; a.click(); 
}
function downloadSrt() {
    let c = ""; subtitleData.forEach((s, i) => c += `${i + 1}\r\n${secToTime(s.start)} --> ${secToTime(s.end)}\r\n${s.text.replace(/<br>/g, '\n')}\r\n\r\n`);
    const b = new Blob([c], {type:"text/plain"});
    const a = document.createElement("a"); a.href = URL.createObjectURL(b); 
    a.download = (document.getElementById('targetFilename').value || "sub").replace('.json','') + ".srt"; a.click();
}
