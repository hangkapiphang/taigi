...
window.onload = function() {
    ...
    // 3. Auto-load ID if present in URL (passed from editor.html)
    const urlParams = new URLSearchParams(window.location.search);
    const fileId = urlParams.get('id');
    if (fileId) {
        document.getElementById('targetFilename').value = fileId;
        // Small delay to let other things settle, then fetch
        ...
        c.innerHTML += `
            <div class="sub-card" id="card-${i}">
                <div class="card-header">
                    ...
                    <button class="btn-icon" style="color:green" onclick="player.seekTo(${s.start}, true)">▶</button>
                    <button class="btn-icon" style="color:red" onclick="deleteLine(${i})">✖</button>
                </div>
