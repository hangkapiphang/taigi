        ...
        if (videoMeta && videoMeta.yt) {
            ...
            player = new YT.Player('player', {
                ...
                events: { 
                    ...
                    'onStateChange': onPlayerStateChange // DETECT END
                }
            });
        }
    }, 100);
...
function showEndSlide() {
    ...
    if (slide && linkEl && videoMeta && videoMeta.yt) {
...
function updateSubtitle(time) {
    ...
    if (currentLine) {
        ...
        if (s.boxColor) {
            ...
            const op = s.boxOp !== undefined ? s.boxOp : 0.5; // Default to 0.5 to match look
            if(rgb) styles.push(`background-color: rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${op})`);
        }
        if (s.shadow && s.shadow > 0) {
            const sc = s.shadowColor || "#000000";
            styles.push(`text-shadow: ${s.shadow}px ${s.shadow}px ${s.shadow}px ${sc}`);
        }
