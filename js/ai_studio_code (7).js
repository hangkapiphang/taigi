...
let player;
let subtitleData = []; // Stores the actual text and time
...
window.onload = function() {
    ...
    // Auto-load ID if present in URL
...
function loadVideo(url) {
    ...
    if(player) {
        ...
        player = new YT.Player('player', { 
            videoId: id,
            events: {
                // IMPORTANT: This starts the "heartbeat" to check time
                'onReady': function(e) { setInterval(checkTime, 100); }
            }
        });
    }
}
...
function updatePreview() {
    ...
    // Apply Font
    if(document.getElementById('subFont')) {
        s.style.fontFamily = document.getElementById('subFont').value;
    }
