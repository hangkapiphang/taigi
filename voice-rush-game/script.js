...
// ==========================================
const ACTIVE_KEY = 'vr_arcade_active_final';
const LIBRARY_KEY = 'vr_arcade_lib_final';
const LEADERBOARD_KEY = 'vr_arcade_lb_final';
...
function confirmName() {
    ...
    // Auto-start Mic for Teacher to Listen
    if(!localStream && peer) navigator.mediaDevices.getUserMedia({ audio: true }).then(s => { 
        localStream = s; 
        if(conn) peer.call(conn.peer, s); 
        document.getElementById('live-mic-dot').classList.add('on');
    }).catch(e => alert("Mic denied! Teacher cannot hear you."));
...
// ==========================================
// 6. UI RENDERERS (LISTS)
// ==========================================
function renderActiveList() {
    const container = document.getElementById('list-container');
    container.innerHTML = currentDeck.length ? '' : '<div style="text-align:center;padding:10px;color:#999;">List is Empty</div>';
    
    currentDeck.forEach(a => {
        const div = document.createElement('div');
        div.className = `animal-item ${a.audio ? 'has-audio' : ''}`;
        
        let controlHtml = '';
        
        if (currentLanguage === 'tw') {
            // Taigi: Show Record/Play Buttons
            if(a.audio) {
                controlHtml = `
                    <button class="btn-mic has-data" onclick="playRecording(${a.id})">▶</button>
                    <button class="btn-mic" onclick="clearRecording(${a.id})">✖</button>
                `;
            } else {
                controlHtml = `<button class="btn-mic" id="btn-rec-${a.id}" onclick="startRecording(${a.id})">🎤 REC</button>`;
            }
        } else {
            // English: Show Test Speaker
            controlHtml = `<button class="btn-mic" style="background:#3498db;" onclick="speak({name:'${a.name}'})">🔊 Test</button>`;
        }

        div.innerHTML = `
            <div class="animal-info">
                <div class="animal-icon">${a.icon}</div>
                <div><div class="animal-name">${a.name}</div></div>
            </div>
            <div class="rec-controls">
                ${controlHtml}
                <button class="btn-del" onclick="deleteAnimal(${a.id})">✖</button>
            </div>`;
...
function renderLibrary() {
    ...
    container.innerHTML = names.length ? '' : '<div style="text-align:center;padding:10px;color:#999;">No saved decks</div>';
    
    names.forEach(name => {
        const div = document.createElement('div');
        div.className = 'library-item';
        
        // Hide delete button for Master (GitHub) files
        ...
        div.innerHTML = `
            <div>
                ...
                <div class="lib-count">${deckLibrary[name].length} items</div>
            </div>
            <div>
                <button class="btn-load" onclick="loadDeck('${name}')">LOAD</button>
                ${delBtn}
            </div>`;
        container.appendChild(div);
    });
    ...
    peer.on('open', () => {
        ...
        peer.on('call', call => {
            ...
            call.on('stream', s => { 
                ...
                aud.srcObject = s; 
                aud.play().catch(e=>console.log("Audio play needs interaction"));
            });
        });
    });
}
...
function syncToRemote(type, data) { if(conn && conn.open) { if(type === 'card') conn.send({type:'update_card', icon:data.icon, name:data.name}); if(type === 'score') conn.send({type:'score_update', score:data}); if(type==='status') conn.send({type:'game_status', msg:data}); } }
...
function stopRecording(id){ if(mediaRecorder){ mediaRecorder.stop(); const b=document.getElementById(`btn-rec-${id}`); b.classList.remove('recording'); b.onclick=()=>startRecording(id); }}
function playRecording(id) { const item = currentDeck.find(x => x.id === id); if(item && item.audio) new Audio(item.audio).play(); }
